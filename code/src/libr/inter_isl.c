...
/* complete input function (for external calls) */
int InputString( unsigned char * );
   ...
   switch ( ISLWhich() ) {
	      ...
	      case PRINTABLE :  { for ( ii = 0; ii < InputGot->NofChars; ii++){
					 ...
					 for ( cptr = (StrBuff.cbase+StrBuff.length); cptr > StrBuff.crt;cptr--)printf("\b");
	                            ...
	                            if ( (InputGot->c[ii] >= CRKEY) && (InputGot->c[ii] <= 13) ) {
				         StrBuff.crt = NULL;
					 fseek(stdin,ii-InputGot->NofChars+1,SEEK_CUR);
#ifndef sun
					 printf("\n");
					 fflush(stdout);
#endif
...
int InputString( unsigned char *ExtString ){
